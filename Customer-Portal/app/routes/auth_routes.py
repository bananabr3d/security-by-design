...
# Packages for Flask
from flask import request, render_template, redirect, url_for, flash, make_response, Response, g
...
def register():
    ...
    if request.method == 'POST' and g.jwt_authenticated == False:
        ...
        if not validate_email(request.form['email']) or not validate_username(request.form['username']) or not validate_password(request.form['password']) or not validate_password(request.form['password2']):
            return render_template('register.html', jwt_authenticated=g.jwt_authenticated)
    ...
    elif request.method =='GET' and g.jwt_authenticated == False:       
        return render_template('register.html')
    
    # Check if user is logged in (JWT is present)
    elif g.jwt_authenticated == True:
...
def login():
    ...
    if request.method == 'POST' and g.jwt_authenticated == False:
        ...
        if not validate_username(request.form['username']) or not validate_password(request.form['password']):
            return render_template('login.html', jwt_authenticated=g.jwt_authenticated)
...
def register_2fa():
    ...
    '''
    # Check if the user has already 2fa activated #TODO
    if check_2fa_activated(twofa_activated=g.user.get_attribute('twofa_activated')):
    ...
    if request.method =='GET':
        ...
        return render_template('register_2fa.html', secret=secret, img_qrcode_data=img_qrcode_data, jwt_authenticated=g.jwt_authenticated)
    
    elif request.method =='POST':
        # Validate otp
        if not validate_otp(request.form['otp']):
            return render_template('register_2fa.html', jwt_authenticated=g.jwt_authenticated)
...
def login_2fa():
    ...
    if not check_2fa_activated(twofa_activated=g.user.get_attribute('twofa_activated')):
        flash('You have no 2FA activated', 'error')
        return redirect(url_for('register_2fa'))
    ...
    # Check Request Method
    if request.method =='GET':
        return render_template('login_2fa.html', jwt_authenticated=g.jwt_authenticated)
...
def refresh_expiring_jwts(response):
    ...
    try:
        ...
        if (target_timestamp > exp_timestamp):
            # Check if user is 2fa authenticated
            try:
                if check_2fa(twofa_activated=g.user.get_attribute('twofa_activated'), jwt_token=get_jwt()) == None:
